# Initial Agency client
NodeJS implementation of Initial Agency client implementing Client2Agency LibInitial protocol V3. 

In a typical application, you'll likely use LibInitial library as an Agency client - a user friendly way to communicate 
...
access to LibInitial Agency API.  
 
 # Initial Client2Agency Protocol
Initial agency protocol message types can be grouped into few areas:
- Onboarding
...
- Initial Client can have relationships (connections) with many parties. For each of these connections, the client needs
...
## Get messages
- Initial Client can download messages from a Connection Agent, hence download messages for particular relationship.
```
...
- Initial Agency stores messages received by cloud agents. With each message it also stores some metadata. One
  particular piece of this metadata is `statusCode` - a flag signalling whether (or how) was given message
  processed. It's Initial Client's responsibility to maintain message status codes and correctly interpret their
  meaning. 
